...
The first exposure in that office has nothing to do with clever attacks — it is the equipment arriving insecure by default.

### Changing Default Passwords

The router ships with a factory username and password, and those defaults are public knowledge for any given make and model. Left unchanged, that is effectively no password at all, since anyone who identifies the device already knows how to log into it. Changing default passwords closes that gap immediately.
...
Together, these two steps are **device hardening** — reducing how much of each device is exposed before anything else is even considered.
...
MAC filtering applies the same principle more broadly, allowing or denying devices across the network against a defined list of permitted MAC addresses rather than locking down one single port.

> [!NOTE]
> A MAC address can be spoofed, so MAC filtering raises the bar rather than acting as an absolute barrier on its own.
...
The fix is dividing the network into **zones** by how much each part is trusted. The staff laptops sit in the **trusted** internal zone; the internet outside is **untrusted** by default. The web server goes into a **screened subnet** — often still called a DMZ — deliberately placed between the two. It stays reachable from outside, but it is walled off from the internal network, so a compromise of that exposed server does not hand the attacker the staff machines along with it. This is the same idea behind [designing a topology so different parts of a network cannot freely reach each other]({{< ref "posts/the-shape-of-a-network" >}}), just applied at the scale of one small office rather than a whole enterprise network.

{{< mermaid >}}
graph TD
    Internet[Internet — Untrusted]
    Firewall[Firewall / Security Rules]
    DMZ[Web Server — Screened Subnet]
    Trusted[Trusted Internal Network]
    Laptops[Staff Laptops]
    Reception[Reception Port]
...
Access control lists are the rules that explicitly permit or block traffic based on criteria like port or address — for instance, allowing outside traffic to reach the web server on the ports it serves, while blocking any attempt from that same server to reach back into the trusted internal network.
