...
Physical security is the layer that exists before any network or login is even reached — controlling who can physically get to a device, a room, or a whole site in the first place. Two of the most common tools are simple, but worth naming properly:

### Locks

The most basic form — badge readers, keypads, or traditional keys, all doing the same job of deciding who can physically open a door. The restricted areas at work are exactly this: a badge is a form of lock, just one that can be selectively granted or revoked per person rather than requiring a physical key to be cut and handed out.

### Cameras

Do not stop anyone getting in, but they record who did, which matters for a different reason — deterrence, and evidence after the fact if something does go wrong. A locked door and a camera solve two different problems: one prevents access, the other creates accountability for whoever had it.
...
A fake system, deliberately built to look like a real, valuable target — a server that appears to hold something worth stealing, but actually holds nothing of value and exists purely to be attacked.

> [!NOTE]
> Anyone interacting with a honeypot is, by definition, doing something they should not be, since no legitimate user has any reason to be there at all. That makes it a way of detecting an intruder with near-total certainty — there is no such thing as a false positive on a system nobody should ever touch.

That makes a honeypot a way of studying exactly how an attacker operates once they think they have gotten somewhere real.

### Honeynet

The same idea scaled up — not one fake server, but a whole fake network of them, built to look like a genuine environment worth exploring in depth, giving a much fuller picture of how an attacker moves once they believe they are inside something real.
...
{{< mermaid >}}
graph TD
    V[Vulnerability — a weakness that exists]
    T[Threat — something capable of exploiting it]
    E[Exploit — the specific method used]
    R[Risk — likelihood combined with impact]
    V --> T
    T --> E
    E --> R
{{< /mermaid >}}
