...
{{< mermaid >}}
graph TD
    A[Port fault reported] --> B[Check the Cable]
    B --> C{Fault found?}
    C -->|Yes| F[Fix and verify]
    C -->|No| D[Check the Interface]
    D --> E{Fault found?}
    E -->|Yes| F
    E -->|No| G[Check the Hardware]
    G --> F
{{< /mermaid >}}
...
Once the cause is confirmed, the fix follows directly: re-terminate the connector properly, keeping each pair twisted as close to the contact points as the standard allows. Reconnected, the CRC error count stops climbing, and the port holds a stable connection under the same load that was causing drops before.
