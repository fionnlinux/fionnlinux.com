...
sequenceDiagram
    ...
    Device->>Server: Discover — "is anything out there?"
    Server->>Device: Offer — an available address and settings
    Device->>Server: Request — "I'll take that offer"
    Server->>Device: Acknowledge — confirmed, you're configured
{{< /mermaid >}}
...
Within a scope, certain addresses can be carved out entirely using **exclusions** — addresses the server is told never to hand out, typically because something on the network already uses that address permanently and manually, and a collision would break it.
...
The extra settings bundled into a DHCP response beyond just the address itself — things like which DNS servers to use, which domain name to append, or which time server to point at. The address is the headline, but the options are doing a lot of the actual configuration work.
...
{{< mermaid >}}
sequenceDiagram
    participant Device as Device (Subnet B)
    participant Relay as Relay Agent (Router)
    participant Server as DHCP Server (Subnet A)
    Device->>Relay: Broadcast — "is anything out there?"
    Relay->>Server: Forwarded directly, not a broadcast
    Server->>Relay: Offer / Acknowledge
    Relay->>Device: Relayed back
{{< /mermaid >}}
...
> [!NOTE]
> This is the same category of problem [DNSSEC solves for DNS]({{< ref "posts/the-beauty-of-dns" >}}) — confirming a response is authentic, not just present. Different protocol, same underlying gap: neither DNS nor NTP originally had any way to check that a response genuinely came from where it claimed to.
